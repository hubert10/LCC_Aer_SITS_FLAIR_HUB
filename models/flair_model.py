...
class FLAIR_HUB_Model(nn.Module):
    ...
    def __init__(self, config: dict, img_input_sizes: dict):
        ...
        self.channels_dict = {
            mod: (
                1
                if mod in ["AERIAL-RLT_PAN", "DEM_ELEV"]
                else (
        ...
        if config["modalities"]["inputs"]["DEM_ELEV"]:
            self.channels_dict["DEM_ELEV"] = (
                1
                if config["modalities"]["pre_processings"]["calc_elevation"]
                and not config["modalities"]["pre_processings"][
        ...
        if any(
            config["modalities"]["inputs"].get(key, False) for key in self.multi_keys
        ):
            if (
                self.task_nclasses
                != config["models"]["multitemp_model"]["out_conv"][-1]
            ):
                config["models"]["multitemp_model"]["out_conv"].append(
                    self.task_nclasses
                )

        if self.encoders:
            mono_encoder_out_channels = next(
                iter(self.encoders.values())
            ).seg_model.out_channels
            config["models"]["multitemp_model"]["encoder_widths"] = (
                self.adjust_fm_length(config, mono_encoder_out_channels)
            )
            config["models"]["multitemp_model"]["decoder_widths"] = (
                self.adjust_fm_length(config, mono_encoder_out_channels)
            )
        ...
        for mod in self.multi_keys:
            if config["modalities"]["inputs"].get(mod, False):
                self.encoders[mod] = UTAE(
                    ...
                    encoder_widths=config["models"]["multitemp_model"][
                        "encoder_widths"
                    ],
                    decoder_widths=config["models"]["multitemp_model"][
                        "decoder_widths"
                    ],
    ...
    def adjust_fm_length(self, config: dict, mono_temp_backbone_channels: list) -> list:
        ...
        if len(mono_temp_backbone_channels) > 2 and (
            mono_temp_backbone_channels[0] == 0 or mono_temp_backbone_channels[1] == 0
        ):
        ...
        expanded_widths = np.linspace(min_value - 1, max_value + 1, target_size).astype(
            int
        )
    ...
    def print_model_parameters(
        ...
        for key, model in encoders.items():
            if model is not None:
                ...
                if key in mono_keys:
                    ...
                    encoder = "Unknown"
                table += "| {:<37} | {:<35} | {:<17} | {:>13,} |\n".format(
                    key, encoder, "backbone", num_params
                )
        ...
        for key, model in decoders_aux.items():
            if model is not None:
                ...
                total_params += num_params
                table += "| {:<37} | {:<35} | {:<17} | {:>13,} |\n".format(
                    key, decoder_arch, "aux loss decoder", num_params
                )
        ...
        for key, model in decoders_main.items():
            if model is not None:
                if has_mono_key:
                    decoder_arch = config["models"]["monotemp_model"]["arch"].split(
                        "-"
                    )[1]
                else:
                ...
                total_params += num_params
                table += "| {:<37} | {:<35} | {:<17} | {:>13,} |\n".format(
                    key, decoder_arch, "task decoder", num_params
                )
    ...
    def calc_backbones_channels(self) -> list[int]:
        ...
        for mod in self.encoders:
            if mod in self.mono_keys:
                ...
                if len(out_channels) > 2 and (
                    out_channels[0] == 0 or out_channels[1] == 0
                ):
        ...
        reversed_decoder_fm = list(
            self.config["models"]["multitemp_model"]["decoder_widths"]
        )[
            ::-1
        ]  # reversed as order given is top to bottom see : https://github.com/VSainteuf/utae-paps/blob/main/src/backbones/utae.py
        for mod in self.encoders:
    ...
    def forward(self, batch: dict, apply_mod_dropout: bool = False) -> dict:
        ...
        for mod, encoder in self.encoders.items():
            if mod in self.mono_keys:
                ...
                logits, fmaps_ = encoder(
                    batch[mod], batch_positions=batch.get(mod.replace("TS", "DATES"))
                )
                logits = self.interpolate_map(logits, img_size)

                logits_tasks[mod] = self.interpolate_map(
                    logits, img_size
        ...
        if apply_mod_dropout and len(self.encoders) > 1:
            modalities_dropout_dict = {
                key: random.uniform(0, 1) for key in fmaps.keys()
            }
        ...
        if active_mono_keys:
            ...
            fused_features = self.fusion_handler(
                logits_tasks, logits_tasks[active_multi_keys[0]]
            )
...
class FusionHandler(nn.Module):
    ...
    def forward(
        self, feature_maps: dict, target_fm_maps: list[torch.Tensor]
    ) -> list[torch.Tensor]:
        """
        ...
        for mod in active_keys:  # Use only active keys
            ...
            if len(mod_fmaps) != len(
                target_shapes
            ):  # Ensure correct number of feature maps
                mod_fmaps = [mod_fmaps[0]] * (
                    len(target_shapes) - len(mod_fmaps)
                ) + mod_fmaps
        ...
        stacked_feature_maps = [
            torch.cat(fmaps, dim=1) for fmaps in zip(*aligned_fmaps)
        ]
